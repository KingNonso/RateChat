...
(function($) {
    ...
    function init(givenOptions) {
        ...
        $browseFileButton.on('change', function() {
            ...
            var last_choice = $("#previous-choice");
            last_choice.hide();
            var use_previous_image = $("#use_previous_image");
            use_previous_image.val('no');
        });
    ...
    function getImageThumbnailHtml(src,file) {
    ...
    function isValidImageFile(file, callback) {
        ...
        {
            callback(false, 'File is too large (max is ' + convertFromBytes(options.maxFileSize) + ' ).');
            return;
        }
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onload = function(e) {
                    ...
                    $fileTab.prepend(getImageThumbnailHtml(e.target.result,file));
